...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640):
        ...
        s = np.stack([letterbox(x, new_shape=self.img_size)[0].shape for x in self.imgs],
                     0)  # inference shapes
    ...
    def __init__(
        ...
        for i, file in enumerate(pbar):
            ...
            if l.shape[0]:
                ...
                assert (l[:, 1:] <=
                        1).all(), 'non-normalized or out of bounds coordinate labels: %s' % file
...
def load_mosaic(self, index):
    ...
    indices = [index] + [
        random.randint(0,
                       len(self.labels) - 1) for _ in range(3)
    ...
    for i, index in enumerate(indices):
        ...
        if i == 0:  # top left
            ...
            x1a, y1a, x2a, y2a = max(xc - w,
                                     0), max(yc - h, 0), xc, yc  # xmin, ymin, xmax, ymax (large image)
            x1b, y1b, x2b, y2b = w - (x2a - x1a
                                      ), h - (y2a - y1a), w, h  # xmin, ymin, xmax, ymax (small image)
        elif i == 1:  # top right
...
def box_candidates(box1, box2, wh_thr=2, ar_thr=20, area_thr=0.2):  # box1(4,n), box2(4,n)
    ...
    return (w2 > wh_thr) & (h2 > wh_thr) & (w2 * h2 /
                                            (w1 * h1 + 1e-16) > area_thr) & (ar < ar_thr)  # candidates
