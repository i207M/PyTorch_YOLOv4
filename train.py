...
def train(hyp, opt, device, tb_writer=None):
    ...
    assert len(names
               ) == nc, '%g names found for nc=%g dataset in %s' % (len(names), nc, opt.data)  # check
    ...
    if pretrained:
        ...
        state_dict = {
            k: v
            for k, v in ckpt['model'].items() if model.state_dict()[k].numel() == v.numel()
        }
    ...
    for epoch in range(
        ...
        for i, (
            ...
            imgs = imgs.to(device,
                           non_blocking=True).float() / 255.0  # uint8 to float32, 0-255 to 0.0-1.0
            ...
            if rank in [-1, 0]:
                ...
                s = ('%10s' * 2 + '%10.4g' *
                     6) % ('%g/%g' % (epoch, epochs - 1), mem, *mloss, targets.shape[0], imgs.shape[-1])
        ...
        if rank in [-1, 0]:
            ...
            if tb_writer:
                tags = [
                    'train/giou_loss', 'train/obj_loss', 'train/cls_loss', 'metrics/precision',
                    'metrics/recall', 'metrics/mAP_0.5', 'metrics/mAP_0.5:0.95', 'val/giou_loss',
            ...
            if save:
                with open(results_file, 'r') as f:  # create checkpoint
                    ckpt = {
                        ...
                        f.read(),
                        'model':
                        ema.ema.module.state_dict() if hasattr(ema, 'module') else ema.ema.state_dict(),
                        'optimizer':
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--hyp', type=str, default='', help='hyperparameters path, i.e. data/hyp.scratch.yaml'
    )
